...
MAX_TENTATIVAS_GOOGLE = 5
BACKOFF_GOOGLE = (1, 2, 4, 8, 12)
...
def _conectar():
    """Abre a planilha com retry e recria a sessão gspread quando necessário."""
    global _planilha, _abas
    with _lock:
        ...
        ultimo_erro = None
        for tentativa in range(1, MAX_TENTATIVAS_GOOGLE + 1):
            try:
                cliente = gspread.authorize(_credenciais())
                planilha = cliente.open_by_key(PLANILHA_ID)
                _planilha = planilha
                print(f"✅ Conexão Google Sheets estabelecida (tentativa {tentativa}).", flush=True)
                return planilha
            except Exception as e:
                ultimo_erro = e
                _planilha = None
                _abas.clear()
                if tentativa >= MAX_TENTATIVAS_GOOGLE:
                    break
                espera = BACKOFF_GOOGLE[min(tentativa - 1, len(BACKOFF_GOOGLE) - 1)]
                print(
                    f"⚠️ Google Sheets indisponível (tentativa {tentativa}/{MAX_TENTATIVAS_GOOGLE}): {e}",
                    flush=True,
                )
                print(f"🔄 Nova tentativa em {espera}s...", flush=True)
                time.sleep(espera)

        raise RuntimeError(f"Falha ao conectar ao Google Sheets após {MAX_TENTATIVAS_GOOGLE} tentativas: {ultimo_erro}")

def _operar_google(func, descricao="operação"):
    """Executa uma operação gspread com retry e recriação da sessão."""
    global _planilha, _abas
    ultimo = None
    for tentativa in range(1, MAX_TENTATIVAS_GOOGLE + 1):
        try:
            ...
            if tentativa >= MAX_TENTATIVAS_GOOGLE:
                break
            espera = BACKOFF_GOOGLE[min(tentativa - 1, len(BACKOFF_GOOGLE) - 1)]
            print(f"⚠️ Falha Google Sheets em {descricao} ({tentativa}/{MAX_TENTATIVAS_GOOGLE}): {e}", flush=True)
            print(f"🔄 Reconectando em {espera}s...", flush=True)
            time.sleep(espera)
            _conectar()
    raise RuntimeError(f"Falha Google Sheets em {descricao} após {MAX_TENTATIVAS_GOOGLE} tentativas: {ultimo}")
...
def _aba(nome):
    with _lock:
        ...
        def abrir():
            ...
            return aba
        aba = _operar_google(abrir, f"abrir aba {nome}")
        _abas[nome] = aba
        return aba
...
def registros(nome, force=False):
    with _lock:
        ...
        if not force and hit and agora_ts - hit[0] < CACHE_TTL:
            return hit[1]
        dados = _operar_google(
            lambda: _aba(nome).get_all_records(numericise_ignore=["all"], default_blank=""),
            f"ler aba {nome}",
        )
...
def init_db():
    """Inicializa as abas. Seguro para chamadas repetidas no mesmo processo."""
    try:
        planilha = _conectar()
        ...
        print(f"✅ Google Planilhas OK ({planilha.title})", flush=True)
        return True
    except Exception as e:
