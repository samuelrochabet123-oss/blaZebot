...
def init_web_db():
    ...
    try:
        with conn.cursor() as cur:
            ...
            for col in ["inicio_sessao", "ultima_rodada_sinal", "sinal_base_id", "ciclo_ativo", "ciclo_id", "tentativa_atual", "ciclo_cor_regra", "ciclo_cor_entrada", "ciclo_estrategia"]:
                typ = "TIMESTAMP" if "sessao" in col else ("INTEGER" if "id" in col or "tentativa" in col else "VARCHAR(150)")
                if col == "ciclo_ativo": typ = "BOOLEAN DEFAULT FALSE"
                cur.execute(f"ALTER TABLE bot_estado ADD COLUMN IF NOT EXISTS {col} {typ};")
            
            cur.execute("""
                DO $$                 BEGIN
                    IF EXISTS (
                        SELECT 1 FROM information_schema.columns
                        WHERE table_name='bot_estado' AND column_name='sinal_ativo' AND data_type='boolean'
                    ) THEN
                        ALTER TABLE bot_estado ALTER COLUMN sinal_ativo TYPE VARCHAR(150)
                        USING CASE WHEN sinal_ativo THEN 'ATIVO' ELSE NULL END;
                    END IF;
                END $$;
            """)
            cur.execute("INSERT INTO bot_estado (id, motor_ativo) VALUES (1, FALSE) ON CONFLICT (id) DO NOTHING;")
...
def mapear_cor_letra(cor_str):
    ...
    if "VERMELHO" in cor or cor in ["RED", "R", "V", "VI"]: return "R"
    if "PRETO" in cor or cor in ["BLACK", "P", "B"]: return "P"
    if "BRANCO" in cor or cor == "WHITE" or cor == "W": return "W"
    return None
...
def cor_info(color, cor_texto=None):
    ...
    if color == 2: return {"sigla": "P", "nome": "PRETO", "classe": "black", "emoji": "⚫"}
    texto = (cor_texto or "").upper().strip()
    if "VERMELHO" in texto or texto in {"RED", "R", "V", "VI"}: return {"sigla": "R", "nome": "VERMELHO", "classe": "red", "emoji": "🔴"}
    if "PRETO" in texto or texto in {"BLACK", "P", "B"}: return {"sigla": "P", "nome": "PRETO", "classe": "black", "emoji": "⚫"}
...
def consultar_dashboard():
    vazio = {
        ...
        "motor": {
            ...
            "ciclo_ativo": False, "tentativa": 0, "cor_regra": None, "cor_entrada": None,
        },
        "historico_sinais": [],
    }
    conn = get_db_connection()
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                SELECT motor_ativo, sinal_ativo, cor_sinal, ultima_estrategia, inicio_sessao, ciclo_ativo,
            ...
            if inicio_sessao:
                cur.execute("""
                    SELECT COUNT(*) FILTER (WHERE resultado = 'WIN') AS wins,
                           COUNT(*) FILTER (WHERE resultado = 'LOSS') AS losses,
                           COUNT(*) FILTER (WHERE resultado = 'PENDENTE') AS pendentes,
                           COUNT(*) AS total
                    FROM estrategia_sinais WHERE criado_em >= %s;
                """, (inicio_sessao,))
            else:
                cur.execute("SELECT 0 AS wins, 0 AS losses, 0 AS pendentes, 0 AS total;")
            wins, losses, pendentes, _total = cur.fetchone()
            wins, losses, pendentes = int(wins or 0), int(losses or 0), int(pendentes or 0)
            profit = (wins - losses) * 1.0

            sinal = None
            if inicio_sessao and motor_ativo and ciclo_ativo:
                cur.execute("""
                    SELECT estrategia, cor_prevista, cor_regra, cor_entrada, tentativa, valor_aposta
                    FROM estrategia_sinais
                    WHERE criado_em >= %s AND resultado = 'PENDENTE'
            ...
            if sinal:
                ...
                vazio["motor"] = {
                    "ativo": motor_ativo, "sinal": True, "cor": cor_sinal,
                    ...
                    "ciclo_ativo": True, "estrategia": estrategia_curta(estrategia_ativa),
                    "wins": wins, "losses": losses, "pendentes": pendentes, "profit": profit,
                }
            else:
                vazio["motor"] = {
                    ...
                    "tentativa": tentativa_atual if ciclo_ativo else 0, "valor_aposta": 1.0,
            ...
            if inicio_sessao:
                cur.execute("""
                    SELECT roll, color, cor, rodada_id FROM blaze_historico
                    WHERE status = 'complete' AND coletado_em >= %s
            ...
            for roll, color, cor, rodada_id in rows:
                vazio["jogos"].append({"roll": roll, "color": color, "cor": cor, "rodada": rodada_id, **cor_info(color, cor)})
            ...
            if inicio_sessao:
                cur.execute("""
                    ...
                    FROM estrategia_sinais WHERE criado_em >= %s ORDER BY id DESC LIMIT 12;
                """, (inicio_sessao,))
                for est, prev, base, res_rod, res_cor, res, criado, tent, c_id, c_regra, c_entr, v_apost in cur.fetchall():
                    if res == "WHITE": res = "LOSS"
                    vazio["historico_sinais"].append({
                        "estrategia": estrategia_curta(est), "estrategia_full": est, "prevista": prev, "base": base,
                        "rodada_resultado": res_rod, "cor_resultado": res_cor, "resultado": res, "criado_em": criado,
                        "tentativa": int(tent or 1), "ciclo_id": c_id, "cor_regra": c_regra, "cor_entrada": c_entr or prev,
                        "valor_aposta": float(v_apost or 1.0),
                    })
        conn.close()
        return vazio
    except Exception as e:
...
<title>Blaze Bot V8.0 Golden Patterns</title>
<style>
:root { --bg: #07090d; --panel: #10141b; --panel2: #151a22; --border: rgba(255,255,255,.08); --text: #eef2f7; --muted: #8993a1; --red: #ff4d5d; --green: #23e27c; --yellow: #ffc857; --white: #f4f5f7; }
...
.header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; gap: 15px; flex-wrap: wrap; }
...
.big { font-size: 23px; font-weight: 950; } .small { color: var(--muted); font-size: 11px; margin-top: 5px; }
.green { color: var(--green); } .red { color: var(--red); } .yellow { color: var(--yellow); } .white { color: var(--white); } .black { color: #d7dce3; }
.signal { margin-bottom: 12px; padding: 24px; border-radius: 18px; background: linear-gradient(145deg, rgba(22,27,36,.98), rgba(12,15,21,.98)); border: 1px solid rgba(255,255,255,.10); text-align: center; }
...
.section-title h2 { font-size: 13px; text-transform: uppercase; letter-spacing: .8px; } .section-title span { color: var(--muted); font-size: 11px; }
.performance { display: grid; grid-template-columns: repeat(4, 1fr); gap: 9px; margin-bottom: 12px; }
.metric { text-align: center; padding: 13px 8px; background: rgba(16,20,27,.92); border: 1px solid var(--border); border-radius: 13px; }
.metric .num { font-size: 22px; font-weight: 950; } .metric .m-label { margin-top: 4px; color: var(--muted); font-size: 9px; text-transform: uppercase; font-weight: 900; }
...
.history-row:last-child { border-bottom: 0; } .history-head { color: var(--muted); font-size: 9px; font-weight: 900; text-transform: uppercase; }
...
.result-win { background: rgba(35,226,124,.12); color: var(--green); } .result-loss { background: rgba(255,77,93,.12); color: var(--red); }
...
.pill small { font-size: 8px; opacity: .75; margin-top: 2px; } .pill.red-bg { background: var(--red); color: white; }
.pill.black-bg { background: #262c35; color: white; border: 1px solid #444b55; } .pill.white-bg { background: white; color: #111; }
.footer { text-align: center; color: #56606d; font-size: 9px; padding: 18px 0 4px; }
.control-btn { border: none; padding: 12px 24px; border-radius: 12px; font-weight: 900; cursor: pointer; font-size: 14px; color: white; }
.control-btn.start { background: var(--green); } .control-btn.pause { background: var(--red); }
.empty { padding: 25px; text-align: center; color: var(--muted); font-size: 12px; }
@media (max-width: 900px) { .top-grid { grid-template-columns: repeat(2,1fr); } .performance { grid-template-columns: repeat(2,1fr); } }
@media (max-width: 600px) { body { padding: 12px; } .top-grid { grid-template-columns: 1fr; } .performance { grid-template-columns: repeat(2,1fr); } .history-row { grid-template-columns: 1.4fr .7fr 1fr 1fr; } .hide-mobile { display: none; } }
</style>
</head>
<body>
<div class="container">
    <div class="header">
        ...
        <div class="header-right">
            <form action="/controle_motor" method="POST">
                {% if status.motor.ativo %}<button type="submit" class="control-btn pause">⏸️ PAUSAR</button>{% else %}<button type="submit" class="control-btn start">▶️ INICIAR</button>{% endif %}
            </form>
            <div class="live"><span class="dot"></span>{% if status.conectado %}COLETOR ONLINE{% else %}COLETOR OFFLINE{% endif %}</div>
        </div>
    </div>

    <div class="top-grid">
        <div class="card"><div class="label">Última rodada</div><div class="big">{{ status.ultima_rodada or 'Aguardando...' }}</div><div class="small">Atualização automática a cada 5s</div></div>
        <div class="card"><div class="label">Rodadas da sessão</div><div class="big yellow">{{ status.total_jogos }}</div><div class="small">Desde o último INICIAR</div></div>
        <div class="card"><div class="label">Motor</div>{% if status.motor.ativo %}<div class="big green">🟢 ATIVO</div>{% else %}<div class="big red">🔴 PAUSADO</div>{% endif %}<div class="small">Coleta continua mesmo pausado</div></div>
        <div class="card"><div class="label">Placar</div><div class="big">{{ status.motor.wins }}W / {{ status.motor.losses }}L</div><div class="small">WHITE = LOSS</div></div>
    </div>

    {% if status.motor.ativo and status.motor.sinal %}
    <div class="signal active">
        <div class="eyebrow">🎯 SINAL ATUAL • PRÓXIMA RODADA</div>
        {% if status.motor.cor == 'R' %}<div class="color red">🔴 VERMELHO</div>{% elif status.motor.cor == 'P' %}<div class="color black">⚫ PRETO</div>{% endif %}
        <div class="strategy">{{ status.motor.estrategia or 'Sinal ativo' }}</div>
        <div class="entry">Valor fixo: R$ {{ '%.2f'|format(status.motor.valor_aposta or 1.0) }} • Sem dobrar (Flat Betting)</div>
        <div class="entry">A entrada usa a cor exata da regra (Sem inversão). Em caso de perda, aguarda o próximo padrão.</div>
    </div>
    {% else %}
    <div class="signal">
        <div class="eyebrow">🎯 SINAL ATUAL</div>
        <div class="color" style="font-size:27px">⚪ AGUARDANDO GATILHO</div>
        <div class="strategy">{% if status.motor.ativo %}Nenhum padrão encontrado.{% else %}Motor pausado.{% endif %}</div>
        <div class="entry">A coleta permanece ativa independentemente do motor.</div>
    </div>
    {% endif %}

    <div class="section-title"><h2>📊 Desempenho</h2><span>Sessão atual</span></div>
    <div class="performance">
        ...
        <div class="metric"><div class="num">{{ status.motor.wins + status.motor.losses }}</div><div class="m-label">Sinais resolvidos</div></div>
        <div class="metric"><div class="num yellow">{% set total_res = status.motor.wins + status.motor.losses %}{% if total_res > 0 %}{{ '%.1f'|format(status.motor.wins / total_res * 100) }}%{% else %}—{% endif %}</div><div class="m-label">Taxa de acerto</div></div>
    </div>

    <div class="section-title"><h2>🧾 Últimos sinais</h2><span>Sessão atual</span></div>
    <div class="history">
        <div class="history-row history-head"><div>Estratégia</div><div>Entrada</div><div>Rodada base</div><div>Resultado</div><div class="hide-mobile">Rodada resolvida</div></div>
        {% if status.historico_sinais %}
            {% for s in status.historico_sinais %}
            <div class="history-row">
                <div>{{ s.estrategia }}</div>
                <div>{% if s.cor_entrada == 'R' %}<span class="red">🔴 R</span>{% elif s.cor_entrada == 'P' %}<span class="black">⚫ P</span>{% endif %}<div style="font-size:11px;color:#8993a1;margin-top:4px;">{% if s.cor_regra %}Regra: {{ s.cor_regra }} → Entrada: {{ s.cor_entrada }}{% endif %} • Aposta Fixa</div></div>
                <div>{{ s.base }}</div>
                <div>{% if s.resultado == 'WIN' %}<span class="result-badge result-win">✓ WIN</span>{% elif s.resultado == 'LOSS' %}<span class="result-badge result-loss">✕ LOSS</span>{% endif %}</div>
    ...
    <div class="section-title"><h2>🎲 Últimos resultados</h2><span>Rodadas da sessão</span></div>
    <div class="results">
        {% if status.jogos %}
            {% for jogo in status.jogos %}
            <div class="pill {% if jogo.sigla == 'R' %}red-bg{% elif jogo.sigla == 'P' %}black-bg{% else %}white-bg{% endif %}" title="{{ jogo.rodada }}">{{ jogo.emoji }} {{ jogo.roll }}<small>{{ jogo.sigla }}</small></div>
            {% endfor %}
        {% else %}
    ...
    <div class="footer">Blaze Bot V8.0 • coleta contínua • golden patterns (Regras 1 e 4) • WHITE = LOSS • proteção contra resolução antecipada</div>
</div>
</body>
</html>
"""
    ...
    try:
        with conn.cursor() as cur:
            ...
            if motor_ativo:
                ...
                cur.execute("UPDATE bot_estado SET motor_ativo = FALSE, atualizado_em = CURRENT_TIMESTAMP WHERE id = 1;")
            else:
                print("▶️ Iniciando NOVA SESSÃO estatística.", flush=True)
                cur.execute("""
                    UPDATE bot_estado
                    SET motor_ativo = TRUE, inicio_sessao = CURRENT_TIMESTAMP, ciclo_ativo = FALSE, ciclo_id = NULL,
                        tentativa_atual = 0, ciclo_cor_regra = NULL, ciclo_cor_entrada = NULL, ciclo_estrategia = NULL,
                        sinal_ativo = NULL, cor_sinal = NULL, ultima_estrategia = NULL, rodada_base_sinal = NULL,
