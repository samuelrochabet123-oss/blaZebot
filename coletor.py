...
def main():
    ...
    if not os.environ.get("GOOGLE_SHEETS_ID"):
        print("❌ GOOGLE_SHEETS_ID não encontrada.", flush=True)
        return

    if not db.credenciais_disponiveis():
        print("❌ Credenciais Google não encontradas (GOOGLE_CREDENTIALS_JSON ou GOOGLE_CREDENTIALS).", flush=True)
        return

    print("✅ GOOGLE_SHEETS_ID encontrada no ambiente.", flush=True)

    while rodando:
        if init_db():
            break
        print("⚠️ Google Planilhas ainda indisponível. Nova tentativa em 15s...", flush=True)
        time.sleep(15)

    if not rodando:
        return

    print("✅ Planilha disponível. Collector pode iniciar.", flush=True)
